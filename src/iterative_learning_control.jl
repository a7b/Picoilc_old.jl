...
function (experiment::QuantumExperiment)(
    ...
    for t = 2:T
        ...
        Δt = experiment.ts[t] - experiment.ts[t - 1]
        x = zeros(96)
        x[1:48] .= experiment.integrator(Gₜ * Δt) * Ψ̃[t - 1][1:48]
        x[49:96] .= experiment.integrator(Gₜ * Δt) * Ψ̃[t - 1][49:96]
        Ψ̃[t] = x
    end
    #end
...
mutable struct ILCProblem
    ...
    function ILCProblem(
        ...
        if d2u
            ...
            f = zₜzₜ₊₁ -> begin
                xₜ = zₜzₜ₊₁[1:dims.x]
                ψ1ₜ = xₜ[1:sys.isodim]
                ψ2ₜ = xₜ[sys.isodim .+ (1:sys.isodim)]
                uₜ = zₜzₜ₊₁[dims.x .+ (1:dims.u)]
                xₜ₊₁ = zₜzₜ₊₁[dims.z .+ (1:dims.x)]
                ψ1ₜ₊₁ = xₜ₊₁[1:sys.isodim]
                ψ2ₜ₊₁ = xₜ₊₁[sys.isodim .+ (1:sys.isodim)]
                return vcat(
                    dynamics(ψ1ₜ₊₁, ψ1ₜ, uₜ, Ẑgoal.Δt),
                    dynamics(ψ2ₜ₊₁, ψ2ₜ, uₜ, Ẑgoal.Δt),
                )
            end
        end
        ...
        if use_system_goal
            Ygoal.ys[end] = experiment.g_analytic(sys.ψ̃goal[slice(1, 2sys.isodim)])
        end
    ...
    function ILCProblem(
        ...
        if use_system_goal
            Ygoal.ys[end] = experiment.g(sys.ψ̃goal)
        end
...
function ProblemSolvers.solve!(prob::ILCProblem; io::IOStream=stdout)
    U = prob.Ẑ.actions
    udim = length(U[1])
    ...
    ΔY = Ȳ - prob.Ygoal
    println(io, "ΔY(T) = ", norm(ΔY.ys[end], prob.settings[:norm_p]))
    println(io)
    ...
    while true
        ...
        if prob.settings[:verbose]
            println(io)
            printstyled(io, "iter    = ", k; color=:magenta)
            println(io)
            printstyled(io, "⟨|ΔY|⟩  = ", mean([norm(y, prob.settings[:norm_p]) for y in ΔY.ys]); color=:magenta)
            println(io)
            printstyled(io, "|ΔY(T)| = ", norm(ΔY.ys[end], prob.settings[:norm_p]); color=:magenta)
            println(io)
            println(io)
            println()
        ...
        if norm(ΔyTnext, prob.settings[:norm_p]) >
            ...
            printstyled(io, "   backtracking"; color=:magenta)
            println(io)
            println(io)
            ...
            while norm(ΔyTnext, prob.settings[:norm_p]) >
                ...
                if i > prob.settings[:max_backtrack_iter]
                    println(io)
                    printstyled(io, "   max backtracking iterations reached"; color=:magenta)
                    println(io)
                    println(io)
                    println()
                ...
                println(io)
                printstyled(io, "       bt_iter     = ", i; color=:cyan)
                println(io)
                printstyled(io, "       min |ΔY(T)| = ", minimum(ΔyT_norms); color=:cyan)
                println(io)
                printstyled(io, "       |ΔY(T)|     = ", norm(ΔyTnext, prob.settings[:norm_p]); color=:cyan)
                println(io)
                println(io)
        ...
        traj = prob.Ẑ
        ys = prob.Ȳs
        @save "Z:/Pico.jl/data/ilc/$(prob.exp_name)_iter_$(k).jld2" traj ys
        k += 1
    end
