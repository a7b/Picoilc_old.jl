...
function (integrator::FourthOrderPade)(
    ...
    Gₜ = G(aₜ, integrator.G_drift, integrator.G_drives)
    Id = I(size(Gₜ, 1))
    isodim = integrator.isodim
    # return (Id - Δt / 2 * Gₜ + Δt^2 / 9 * Gₜ^2) * ψ̃ₜ₊₁ -
    #        (Id + Δt / 2 * Gₜ + Δt^2 / 9 * Gₜ^2) * ψ̃ₜ
    return [(Id + Δt^2 / 9 * Gₜ^2) * (ψ̃ₜ₊₁[1:isodim] - ψ̃ₜ[1:isodim]) -
        Δt / 2 * Gₜ * (ψ̃ₜ₊₁[1:isodim] + ψ̃ₜ[1:isodim]);(Id + Δt^2 / 9 * Gₜ^2) * (ψ̃ₜ₊₁[1+isodim:2*isodim] - ψ̃ₜ[1+isodim: 2*isodim]) -
        Δt / 2 * Gₜ * (ψ̃ₜ₊₁[1+isodim:2*isodim] + ψ̃ₜ[1+isodim:2*isodim])]
end
